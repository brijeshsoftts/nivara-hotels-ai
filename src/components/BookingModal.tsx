...
export const BookingModal: React.FC<BookingModalProps> = ({
  ...
  }, [initialRoom, initialRoomId, initialCheckIn, initialCheckOut, initialGuests]);

  const currentRoom = ROOMS_DATA.find((r) => r.id === selectedRoomId) || ROOMS_DATA[1];
      ...
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto p-6 sm:p-8">
        {isSuccess ? (
          <div className="text-center py-6 px-2 space-y-6">
            <div className="w-16 h-16 mx-auto rounded-full bg-primary/20 border border-primary flex items-center justify-center text-primary">
            ...
            <div className="space-y-2">
              ...
              <DialogDescription className="text-sm text-muted-foreground max-w-md mx-auto">
                Your reservation dossier <span className="text-primary font-medium font-mono">#{bookingCode}</span> has been transmitted to our Chief Concierge. A personalized itinerary will be dispatched to {email || 'your email'}.
              </DialogDescription>
            </div>

            <div className="bg-card p-5 rounded-2xl border border-border text-left text-xs space-y-2 text-muted-foreground">
              <div className="flex justify-between">
                <span>Sanctuary:</span>
                <span className="text-foreground font-serif font-medium">{currentRoom.name}</span>
              </div>
              <div className="flex justify-between">
                <span>Arrival:</span>
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Room Selection */}
              <div>
                <Label className="block text-[11px] uppercase tracking-[0.16em] text-muted-foreground mb-2 font-medium">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                  {ROOMS_DATA.map((room) => (
                    <Button
                      ...
                      className={`h-auto flex items-center justify-between p-3 rounded-xl text-left border transition-all whitespace-normal ${
                    ...
                    >
                      <div className="space-y-0.5">
                        <p className="text-xs font-serif font-medium text-foreground">{room.name}</p>
                        <p className="text-[10px] text-muted-foreground">{room.sizeM2} m² · {room.view}</p>
                      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="flex h-9 w-full rounded-md border border-input bg-card px-3 py-1 text-xs text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring cursor-pointer"
                  >
                    <option value={1} className="bg-card text-card-foreground">1 Guest (Solitary)</option>
                    <option value={2} className="bg-card text-card-foreground">2 Guests (Standard)</option>
                    <option value={3} className="bg-card text-card-foreground">3 Guests</option>
                    <option value={4} className="bg-card text-card-foreground">4 Guests (Pavilion)</option>
                    <option value={6} className="bg-card text-card-foreground">6 Guests (Residence)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-3 border-t border-border">
                <div className="text-left w-full sm:w-auto">
                  <p className="text-[11px] text-muted-foreground">
                    Total for {nights} night{nights > 1 ? 's' : ''} (Includes Taxes & Valet):
                  </p>
